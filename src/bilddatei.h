...
/* Zur �bergabe an malBildchen(): Welches Viertel soll gemalt werden?
   Entweder viertel_alle �bergeben oder
   viertel_q** | viertel_z**
...
/* Achtung: Wenn die nachfolgenden Konstanten ge�ndert werden, muss
   auch const_werte in blop.cpp ge�ndert werden. */
...
/* F�r range-Check (siehe BildStapel::speichereBild()) */
...
/**verwaltet ein xpm als Ansammlung von 16x16-Bildchen; und auch sonstige
  *Bilder. K�mmert sich insbesondere um Umf�rbung und Bildschirmskalierung
  */
...
class Bilddatei {
 public: 
  ...
  /* F�r gleiche Quelle aber unabh�ngige Nachbearbeitung.
     Diese wird gleich schon mal in Form einer Umf�rbung vollzogen. */
  Bilddatei(Bilddatei *, const Color &);


  /** L�dt das Bild mit dem angegebenen Namen. Sucht in verschiedenen
  ...
  /** liefert zur�ck, wie viele Bildchen in dieser Datei sind. */
  ...
  /* Malt das angegebene Rechteck (bzw. Teile davon) so oft, dass
     ein horizontaler Streifen der L�nge l entsteht. Geht davon aus,
     das in dem Bildchen das src-Rechteck horizontal einheitlich ist.
     Je gr��er src, desto schneller geht das malen. */
  ...
  /** liefert die Gesamtbreite in Pixeln zur�ck */
  int getBreite() const;
  /** liefert die Gesamthoehe in Pixeln zur�ck */
  int getHoehe() const;
