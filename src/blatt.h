...
/* Men�punkte werden an verschiedenen Linien zentriert. Die Positionen
...
/* Konstanten f�r mausbereich.mEintrag */
#define eintrag_keiner (-1)
#define eintrag_scrollleiste (-2)

/* Der einzige Subbereich, der f�r Tastatur-User existiert. */
#define subbereich_default 0
/* Die folgenden Konstanten sind nur zur �bergabe von oder an
...
struct MausBereich {
  ...
  int mEintrag;
  /* Die Bedeutung von subbereich h�ngt von der Men�eintragart ab.
     Subbereich ist nur f�r Maususer. Aus Sicht von Tastatur-Usern
     gibt es nur den default-Subbereich. (Sobald eine Taste gedr�ckt
...
enum yneuwahl {   /* Zur �bergabe an calcScroll():
...
class BlattMenu: public Blatt {
  ...
  friend class MenuEintrag;  /* Damit Men�punkte doEscape() aufrufen k�nnen */

 protected:
  /* Soll es auch dann eine Scrollleiste haben, wenn sie unn�tig ist?
  ...
  std::vector<int> mEintraegeY;  /* relativ zu mY0.
				    ...
				    so da� es auch die Gesamth�he enth�lt. */
  BlattMenu * mObermenu;    /* NULL f�r das Hauptmen� */
  
  /* Wenn der Eintrag, wo dies ein Untermen� ist, erfahren m�chte, wenn
     das Men� verlassen wird, sollte er sich mit setObereintrag() hier
     eintragen. */
  MenuEintragSubmenu * mObereintrag;

  /* Wenn was Hyperaktiv ist, kann mWahl trotzdem ein anderer Men�punkt
     sein: wenn sich die Maus �ber einem anderen Men�punkt befindet.
     (Tastendr�cke gehen dann aber an den hyperaktiven Eintrag) */
  MausBereich mWahl;
  /* True, wenn Button unten; wird (im Moment) nur f�r Scrollleistenpfeile
     gebraucht. */
  bool mPress;
  int mHyperaktiv;  // eintrag_keiner wenn nix hyperaktiv ist
  Str mInfoText;
  int mInfoW;       // negativ f�r "kein Scrollen", sonst Textbreite
  ...
  /* mX0 = x-Koordinate f�rs alignen... getrennt nach 
     mY0 = y-Koordinate vom oberen Rand von Men�punkt 0
     (evtl. weit oberhalb vom Bildschirm) */
  int mX0[zl_anzahl], mY0, mZeigVon, mZeigBis;
  /* mAnimXX: Wie ohne Anim, aber tats�chliche Position w�hrend einer
 ...
 protected:  
  ...
  /* �ndert mWahl und k�mmert sich drum, dass Graphik geupdatet wird */
  void setWahl(MausBereich wahl);
  void setWahl(int eintrag, int subBereich = subbereich_default);
 
  /* �ndert mHyperaktiv und k�mmert sich drum, dass Graphik geupdatet wird */
  void setHyperaktiv(int ha);

  /* K�mmert sich um mInfotext und mInfoY.
     Wird aufgerufen, wenn sich mInfotext vielleicht �ndern soll.
...
class BlattStartAt: public BlattMenu {
 ...
 public:
  ...
  virtual void oeffnen(bool durchMaus, int wahl = eintrag_keiner);
    /* Hier wird der int ignoriert, da sich die Klasse selbst drum k�mmert */
 protected:
  virtual void doReturn(bool durchMaus);
};
