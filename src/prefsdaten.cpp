...
Modified 2003,2006,2008,2010,2011,2014 by the cuyo developers
...
/** Geschwindigkeit des KI-Players, so wie man's einstellen m�chte */
int mKIGeschwLog;
...
/* The following are stored as strings in .cuyo, but it is safe to use ints
   here, because the semantics can only change between different versions of
   cuyo, not during one invocation. */
int mLevelTrack;
int mDifficulty;



/** L�dt die Preferences aus wo-auch-immer-sie-abgespeichert-werden
in die Variablen. */
...
void init() {
  try {
    ...
    print_to_stderr("Error reading preferences file '.cuyo':\n");
...
void schreibGewonnenenLevel(bool sp2, const Str intlena) {
  if (mGewonneneLevel[sp2].find(intlena)==mGewonneneLevel[sp2].end()) {
    /* Level war bisher noch nie gewonnen; also in Liste einf�gen */
...
Str getPrefsName() {
  ...
  if (!ho) {
    ...
    print_to_stderr("Warning: Env-Variable $HOME not found. Using the current directory for .cuyo");
    return ".cuyo";
  }
  if (ho[strlen(ho) - 1] == '/')
    return Str(ho) + ".cuyo";
...
void liesPreferences() {
  ...
  mPlayers = prd.getZahlEintrag("Players",1);
  mLevelTrack = Version::gLevelpack.suchMerkmal(
    prd.getEintrag("LevelTrack","main"));
  if (mLevelTrack==-1) mLevelTrack=Version::gLevelpack.suchMerkmal("main");
  mDifficulty = Version::gSchwierig.suchMerkmal(
    prd.getEintrag("Difficulty",""));
  if (mDifficulty==-1) mDifficulty=Version::gSchwierig.suchMerkmal("");
  mLastLevel = prd.getEintrag("LastLevel", "");
}
...
void schreibPreferences() {  
  ...
  if (!f) {
    print_to_stderr(_sprintf("Warning: Could not write preferences file \"%s\".",
			     na.data()));
    return;
  }
  ...
  fprintf(f, "LevelTrack=%s\n\n",
    Version::gLevelpack.mMerkmale[mLevelTrack].data());

  fprintf(f, "\n");
  fprintf(f, "Difficulty=%s\n\n",
    Version::gSchwierig.mMerkmale[mDifficulty].data());
...
int getPlayers() { return mPlayers; }
void setPlayers(int n) { mPlayers = n; }

int getLevelTrack() { return mLevelTrack; }
void setLevelTrack(int t) { mLevelTrack = t; }

int getDifficulty() { return mDifficulty; }
void setDifficulty(int d) { mDifficulty = d; }

Str getLastLevel() { return mLastLevel; }
void setLastLevel(const Str l) { mLastLevel = l; }
