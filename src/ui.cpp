...
/* Ist vielleicht zur Zeit etwas unn�tig, eine eigene ui-Zeitschritt-Fkt
...
void run() {
  ...
  while (!mQuit) {
    ...
    /* Alle wartenden Events aus der Queue holen. Wenn eins dieser Events
       ein Graphik-Update fordert, dann wird das sofort ausgef�hrt, auch
       wenn wir in Zeitnot sind. Deshalb von fr�her rumliegende
    ...
    /* Ein bisschen Zeit f�r's Betriebsystem */
    ...
    if (akt_zeit > zeit) {
      ...
      if (!ausfallen) {
        /* Graphik (falls n�tig) updaten. M�glicherweise handelt es sich
           um einen Update, der gar nicht von diesem zeitSchritt()-Aufruf
           gefordert wurde, sondern von einem �lteren */
...
void sofortAllesAnzeigen() {
  ...
  */
  ...
  /* Hier finden die ganzen Area::xxx Malbefehle statt. Die Malroutinen
     sind selbst daf�r verantwortlich, nur das neu zu malen, was sich
     ge�ndert hat. Au�erdem m�ssen sie Area::updateRect() aufrufen f�r
...
void setBlatt(Blatt * b) {
  mBlatt = b;
  /* Der gesamte Bildschirm hat sich ge�ndert, nicht nur das, was das
