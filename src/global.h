...
/* In dieser Datei werden all die Dinge definiert, von denen ich nicht
   wei�, wo ich sie hin tun soll. Unter anderem ein paar globale Variablen. */


/** True, wenn -d �bergeben wurde, d. h. im Debug-Modus. */
extern bool gDebug;
...
/** True, wenn der Benutzer den Namen einer ld-Datei �bergeben hat. */
extern bool gDateiUebergeben;

/** Wenn eine ld-Datei �bergeben wurde, dann ist das der Name davon. */
...
/** Entfernt von p alles, was nach dem ersten Punkt kommt. Ist dazu da
    um aus einem "bla.xpm", was unter pics steht, den Namen f�r Programme
    zu extrahieren. */
Str picsEndungWeg(const Str & p);


/** d sollte ein Pfad mit Dateiname sein. Liefert nur den Pfad-Anteil
    zur�ck (d. h. alles vor dem letzten "/". Liefert "./", falls d keinen
    "/" enth�lt. */
Str nimmPfad(Str d);


/** d sollte ein Pfad mit Dateiname sein. Liefert nur den nicht-Pfad-Anteil
    zur�ck (d. h. alles nach dem letzten "/". Liefert alles, falls d keinen
    "/" enth�lt. */
