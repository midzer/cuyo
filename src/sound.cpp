...
/* Um rauszufinden, ob's ein oder zwei Spieler gibt, f�r Stereo */
#include "cuyo.h"
...
void init() {
  ...
  /* Sound wird nur dann jetzt schon initialisiert, wenn
     ...
     auf denen Sound nicht geht, braucht SDL n�mlich manchmal
...
/** Sollte aufgerufen werden, wenn sich Pref->sound
    m�glicherweise ge�ndert hat */
void checkePrefsStatus() {

  /* M�glicherweise muss die Musik gestoppt werden */
  if (mSoundGeht && !PrefsDaten::getSound())
    setMusic("");
    

  /* M�glicherweise sound-Initialisierung nachholen */
  ...
  if (Mix_OpenAudio(sound_freq, sound_format, 2, sound_chunk_size) < 0) {
    ...
    print_to_stderr(_sprintf("Sound does not work: %s\n", SDL_GetError()));
    return;
  } 
...
/** L�dt den angegebenen Sample und liefert eine Nummer zur�ck,
...
/** Gibt alle Samples wieder frei, die mit ladSample geladen worden
    sind, au�er die, die init() geladen hat. Sollte nach Levelende
...
/** Spielt das Sample mit der angegebenen Nummer (die entweder eine
    der obigen Konstanten ist oder von ladSample zur�ckgeliefert wurde).
    so,xz,xn bestimmen die x-Position f�r Stereo-Effekte.
    Dabei ist xz/xn ein Bruch, 0 f�r den linken und 1 f�r den rechten
