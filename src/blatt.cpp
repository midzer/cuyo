...
void BlattSpiel::malInfos(int sp, int x) {
  ...
  malInfosRand(x, L_naechstesfall_hoehe); /*------------*/
  
  
  Area::fillRect(x, L_naechstesfall_hoehe + L_rand, L_infos_breite, L_punkte_y - 2 * L_rand - L_naechstesfall_hoehe, c);
  Str p =
  ...
  /* Wenn sich die Randfarbe ge�ndert hat, dann Punkte auf
  ...
  Area::fillRect(x, pu, L_infos_breite, L_infoblobs_y - L_rand - pu, c);

  malInfosRand(x, L_infoblobs_y - L_rand); /*------------*/
...
//   Area::leave();
  ...
  Area::fillRect(x + L_infoblobs_breite, L_infoblobs_y, L_infos_breite - L_infoblobs_breite, L_fenster_hoehe - L_infoblobs_y, c);

  if (ld->mLevelGeladen) {
    /* To be cleaned: Instead of being the full height, the following area should rather
     * go from L_infoblobs_y to the bottom */
    Area::enter(SDLTools::rect(x, 0, L_infos_breite, L_fenster_hoehe));
    Spielfeld* spielfeld = Cuyo::getSpielfeld(sp);
    if (ld->mGreysAtAll)
      Font::gGame->drawText(_sprintf("%d", spielfeld->getGrauAnz()),
        L_greygrass_x2,L_grey_y+gric/2,AlignLeft);
    Font::gGame->drawText(_sprintf("%d", spielfeld->getGrasAnz()),
      L_greygrass_x2,L_grass_y+gric/2,AlignLeft);
    ...
    Font::gGame->drawText(PlatzAnzahlFormat, L_levelexplode_x2, L_levelexplode_y + gric / 2, AlignLeft);
    Area::updateRect(L_greygrass_x2, L_infoblobs_y,
                     L_infos_breite-L_greygrass_x2, L_infoblobs_hoehe);
    Area::leave();
  }
}
...
void BlattSpiel::anzeigen() {
  ...
  for (int i = 0; i < spz; i++) {
    ...
    Area::enter(SDLTools::rect(chooseX(L_infos_x, spz, i), 0,
                               L_infos_breite, L_infos_hoehe));
    Cuyo::malNaechstesFall(i);
    Area::leave();
  }
...
void BlattMenu::oeffnen(bool /*durchMaus*/, int wahl /*= eintrag_keiner*/) {
  ...
  /* Nicht die nachfolgenden Setzroutinen verwenden, weil sonst
     evtl. ein nicht-mehr existenter voriger gew�hlter Men�punkt neu
     gemalt wird. */
  ...
  /* Daf�r manuell der Infozeile sagen, dass sie geupdatet werden muss */
  updateInfo();
...
//   }
  ...
  switch (t) {
    ...
    default:
      ...
      /* Vielleicht ist der gew�hlte Eintrag aktiv? */
      if (mWahl.mEintrag >= 0)
...
MausBereich BlattMenu::getMausPos(int x, int y) {
  ...
  /*                ~~ C garantiert, dass die rechte Seite nur
     ausgef�hrt wird, wenn's n�tig ist */
...
void BlattMenu::mouseMotionEvent(bool press, int x, int y, int x_alt, int y_alt) {
  ...
  /* Bei gedr�ckter Maustaste rumfahren
     => Dort, wo der Knopf runter ging bleibt's ausgew�hlt */
  if (press) {
    return;
  }
...
void BlattMenu::mouseButtonEvent(bool press, int x, int y) {
  ...
  if (mWahl.mEintrag >= 0)
    ...
    /* Um ganz hoch oder runter zu scrollen, k�nnen wir einfach auf
...
void BlattMenu::anzeigen() {
  ...
  if (mRaenderUpdaten) {
    ...
    /* Zwischen Men� und Infozeile */
    /* Mark: H�h, ist das nicht das gleiche wie
       mAnimY0+mEintraegeY[mAnimZeigBis] ? */
    /* Immi: Wei� auch nicht mehr. Vielleicht wusste ich nicht, dass
       mEintraegeY eins weiter geht als Gesamtzahl der Eintr�ge */
    int y_u = mAnimY0+mEintraegeY[mAnimZeigBis - 1]
      + mEintraege[mAnimZeigBis - 1]->mHoehe;
    /* Provisorisch: Das Hauptmen� ist zu lang, wenn "debug-mode" drunter steht;
    ...
    /* Wenn diese Farbe ge�ndert wird, mu� das auch in
...
void BlattMenu::zeitSchritt() {
  ...
  /* Men�eintr�ge d�rfen ihre Privat-Animation haben */
  for (size_t i=0; i<mEintraege.size(); i++)
    mEintraege[i]->zeitSchritt();
  
  /* Maus auf Scrollpfeil gedr�ckt? */
  ...
  /* Echte Scrollpos an gew�nschte anpassen */
  scrollZeitSchritt();
}
...
/* �ndert wahl und k�mmert sich drum, dass Graphik geupdatet wird */
...
/* Teilt dem Eintrag seinen neuen Subbereich mit. Der Eintrag k�mmert
...
void BlattMenu::setScrollZielHigh(yneuwahl ynw /*=ynw_mitte*/, bool sprung /* = false */) {
  ...
  /* Kein Men�punkt gew�hlt? Dann nach oben */
  int w = mWahl.mEintrag < 0 ? 0 : mWahl.mEintrag;

  /* Erstmal nehmen wir an, da� wir mWahl gem�� ynw ausrichten wollen */
...
void BlattMenu::setScrollZielLow(int neuy, bool sprung /* = false */) {
  ...
  if (!istScrollbar()) {
    /* Das Men� passt komplett auf den Bildschirm. Dann wird der Scrollwunsch ignoriert. */
    neuy = (L_menu_hoehe-mEintraegeY[mEintraege.size()])/2;
    
  } else {
    /* Wenn scrollbar, dann aber nicht weiter rausscrollen als gesund w�re */
...
void BlattMenu::scrollZeitSchritt() {
  ...
  /* Wie schnell w�ren wir am liebsten, unter Ber�cksichtigung des Bremswegs?
...
void BlattStartAt::oeffnen(bool durchMaus, int) {
  /* Zu umstaendlich:
     Im Moment liefert getMoeglicheLevel einen Vector von Strings zur�ck,
  ...
  /* MenuEintragAuswahl kann hier nicht verwendet werden: MenuEintragAuswahl()
     setzt das Blatt nach Return-Dr�cken auf Obermen� zur�ck; hier muss das
     Blatt aber auf Spiel gesetzt werden.
     (=> Im Moment kommt MenuEintragAuswahl nur noch an einer Stelle vor. Daf�r
     lohnt sich eigentlich keine eigene Klasse.) */

  /* Darf erst jetzt aufgerufen werden; das Men� muss vorher schon aufgebaut
     worden sein */

  int lnr = Cuyo::getLetzterLevel();
  if (lnr != 0 && lnr<=wanz) gewaehlt = lnr;
  /* Wahl z�hlt bei 0 los, level bei 1 */
...
bool stromRestartLastLevel() {
  int lastLevel = Cuyo::getLetzterLevel();
  return lastLevel != 0 && ld->levelAccessible(lastLevel);
}
