...
/***** Debug-Variablen *****/
/* Es gibt au�erdem noch die globale Variable gDebug; */
bool mEinzelschritt;
bool mZeitlupe;
...
/** Das Bild, das angezeigt wird, w�hrend das Spiel auf Pause steht. */
...
/** global-Modus (Spieler�bergreifend). */
enum global_modus {
  gmodus_kein_spiel,
  gmodus_spiel_start, // wie gmodus_spiel, aber Text muss noch gel�scht werden
  gmodus_spiel,
...
bool mWarteAufTaste;
/* Wenn > 0, wird h�chstens so lange auf Taste gewartet */
int mWarteTimeout;
...
/** Aktuelle Level-Nummer, wenn ein Spiel l�uft. Sonst undefiniert. */
int mLevelNr;
/** (Interner) Name des aktuellen Levels. Enth�lt, wenn grade kein Spiel
    l�uft, den Namen vom Level zum weiterspielen. Ist (wenn kein Spiel
    l�uft) "", wenn es keinen gibt. */
Str mIntLevelName;
int mPunkte[max_spielerzahl];
/** Wird nur w�hrend der Zeitbonus-Animation gebraucht... */
int mZeitBonus;
...
void stopLevel();
/** Setzt die Punktzahl f�r Spieler sp */
...
/** stoppt das Spiel sofort (egal, ob grad ein Level l�uft oder nicht) */
void stopSpiel(bool noch_anzeigen = false);
/** Die Haupt-Spielschritt-Routine, w�hrend das Spiel l�uft.
    Ruft alle anderen spielschritt()-Routinen auf. */
void spielSchritt();
/** Wird von startLevel() und von spielSchritt() aufgerufen. L�sst
    s�mtliche Blops animieren. */
void animiere();
...
/** Liefert den Namen und Pfad der Prefs-Datei zur�ck
    ($HOME/.cuyo) */
Str getPrefsName();
/** Liefert den Namen und Pfad der Log-Datei zur�ck
    ($HOME/cuyo.log) */
Str getLogName();
...
void startSpiel(int level) {
  ...
  mLevelNr = level;
  mIntLevelName =
    (mLevelNr>ld->getLevelAnz() ? "" : ld->getIntLevelName(level));
  ...
  try {
    if (!startLevel())
      throw Fehler("%s","There are no (more?) (working?) levels.");
...
bool startLevel() {
  ...
  if (!ld->mLevelGeladen || ld->mLevelNummer != mLevelNr) {
    ...
    /* Ok, neu laden. Erst mal alten Level entladen, damit w�hrend
       der "loading level"-Anzeige nicht noch falsche Informationen
       �ber den alten Level angezeigt werden. */
    ld->entladLevel();
    ...
    /* "Loading Level" gleich anzeigen. Au�erdem den Rand neu malen, da
       wir die Schriftfarbe ge�ndert haben und das Numexplode vom alten
       level ung�ltig ist und so. */
    UI::randNeuMalen();
  ...
  //}
  /* Einige Farben haben sich durch das level-laden ge�ndert. Also
     nach dem Level alles neu malen. Und da die mal-Routine nicht auf
     die Idee kommt, dass die R�nder um den Level neu an den X-Server
     geschickt werden m�ssen, selbst Area::updateAll() aufrufen. */
  UI::randNeuMalen();
	
  /* Aufnehmen / Abspielen starten */
  Aufnahme::init(mAbspielen, getSpielerModus());

  /***** Allen Leuten erz�hlen, dass jetzt ein Level anf�ngt. *****/
  
  /* F�r den Global-Blop... */
  ld->startLevel();

  /* Hier ist die Stelle, wo wir darauf gucken, wie viele Spieler mitspielen;
     nur f�r so viele Spieler wird das Spiel wirklich gestartet */
  for (int i = 0; i < mSpielerZahl; i++) {
  ...
  /* Auch die KI m�chte wissen, wenn ein neuer Level anf�ngt. */
  if (mGegenKI)
    mKI->startLevel();
...
void spielSchritt() {
  ...
  /* Den Schritt aufnehmen bzw. abspielen. Beim Abspielen werden
     hier auch ggf. Tastendr�cke abgespielt. */
  Aufnahme::recSchritt(mSpielfeld);

  /* Ggf. einen Spielschritt f�r die KI */
  if (mGegenKI)
    mKI->spielSchritt();

  
  /*** Die einzelnen Teile eines Spielschritts ausf�hren: ***/
...
#define ALLE_SPIELER for (int i = 0; i < mSpielerZahl; i++) mSpielfeld[i]
  ...
  /* Evtl. zuf�llige Graue an die Spieler senden. (Blops werden nicht
  ...
  /* Neue Explosionen testen. Da dabei keine Blops manipuliert werden, k�nnen
     wir um alles eine gro�e gleichzeit machen. */
...
#undef ALLE_SPIELER
  ...
  if (mGModus == gmodus_spiel) {
    /* Nur wenn das Spiel wirklich noch l�uft (und nicht nur darauf gewartet
       wird, das wir beenden k�nnen: Testen, ob wir gewonnen haben */

    /* Wurde grade das restliche Gras vernichtet? */
    for (int i = 0; i < mSpielerZahl; i++)
      if (mSpielfeld[i]->getGrasAnz()>0)
        goto noch_gras_da;
...
noch_gras_da:;
	...
	/* Gewonnen. Level als gewonnen abspeichern. Aber nicht im
	   Debug-Modus. */
	if (!gDebug)
	  PrefsDaten::schreibGewonnenenLevel(mSpielerZahl > 1, mIntLevelName);
  ...
  } // Ende: if warten, bis alle f�r's Spielende bereit sind
...
#define bonusMessage _("Level %s complete!\n\nTime Bonus: %d\nScore: %d\n\n%s")
  ...
  if (ba_fertig) {
    ...
    mWarteAufTaste = true;
    /* Kein Warte-Timeout mehr. Einfach Leertaste dr�cken */
...
void zeitSchrittIntern() {
  ...
  /* W�hrend Pause l�uft das Spiel nicht weiter */
  if (mPause)
    return;
  
  /* Warten wir grad drauf, dass der Benutzer eine Taste dr�ckt? */
  if (mWarteAufTaste) {
  ...
  if (mGModus == gmodus_bonus_warte) {
    /* Wir haben grade ein bisschen gewartet, damit der Benutzer seine
       Level-Endpunkte bewundern kann. Jetzt geht's weiter mit dem n�chsten
       Level... */
			
    /* Alten Level stoppen (ist vermutlich nicht wirklich n�tig) */
    stopLevel();
    ...
    mLevelNr++;
    mIntLevelName =
      (mLevelNr>ld->getLevelAnz() ? "" : ld->getIntLevelName(mLevelNr));
    try {
      if (!startLevel()) {
      ...
      printFehler(mSpielerZahl, "Could not start level:\n", fe);
      stopSpiel();
    }
    PrefsDaten::setLastLevel(mIntLevelName);
    return;
  ...
  /* Spiel grade erst gestartet? Dann Texte l�schen */
  ...
  if (mGModus == gmodus_bonus_animation) {
    ...
    /* Evtl. Signale abfangen, um die Logdatei abspeichern zu k�nnen. */
    signaleAn();
...
/** Wird von startLevel() und von zeitSchritt() aufgerufen. L�sst
    s�mtliche Blops animieren. */
void animiere() {
  
  Blop::beginGleichzeitig();
  
  /* Alle Grafiken l�schen */
  Blop::lazyLeereStapel();

  /* Erst mal das globale Blop ausf�hren. */
  ld->spielSchritt();
...
/** reSp bittet den anderen Spieler um eine Reihe. Er selbst
    hat H�he h. Antwort ist eine der Konstanten bewege_reihe_xxx */
int bitteUmReihe(bool reSp, int h) {
  return mSpielfeld[!reSp]->bitteUmReihe(h);
}


/** reSp will einen Stein vom anderen Spieler (r�berreihe) */
...
Str getLogName() {
  ...
  if (!ho) {
    ...
    print_to_stderr("Warning: Env-Variable $HOME not found. Using the current directory for cuyo.log");
    return "cuyo.log";
  }
  if (ho[strlen(ho) - 1] == '/')
...
/** liefert true, wenn das Spiel normal l�uft, false
...
/** Liefert die Anzahl der Mitspieler zur�ck. */
int getSpielerZahl() {
  return mSpielerZahl;
}


/** Liefert das Pause-Bildchen zur�ck */
Bilddatei * getPauseBild() {
  return mPauseBild;
}


/** Liefert true, wenn debug-R�berreihen-Test aktiv ist */
bool getRueberReihenTest() {
  return mRueberReihenTest;
}


/** Liefert ein Spielfeld zur�ck. */
...
void init() {
  ...
  mSchwierig = PrefsDaten::getDifficulty();

  int spielermodus = PrefsDaten::getPlayers();
  mGegenKI = spielermodus==spielermodus_computer;
  mSpielerZahl = (mGegenKI ? 2 : spielermodus);

  mLevelpack = PrefsDaten::getLevelTrack();
  ...
  setSpielerModus(spielermodus);
    // Sollte erst nach Erzeugen der Men�s aufgerufen werden...
...
void keyEvent(const SDL_Keysym & taste) {
  ...
  if (mWarteAufTaste) {
    ...
    if (PrefsDaten::getTaste(taste.sym, sp, t)) {
      /* Im 1-Spieler-Modus und im KI-Modus alle Tastendr�cke an
	 Spieler 1 senden: */
...
bool debugKeyEvent(const SDL_Keysym & taste) {
  ...
  if (buch == 'S') {
    print_to_stderr("Debug: Save log file\n");
    Aufnahme::speichern(getLogName());
    return true;
  }

  /* Handle other keys only in debug mode. */
  ...
  if (buch == 'b') {
    ...
    print_to_stderr("Debug: Receive greys\n");
    
  } else if (buch == 'e') {
    mEinzelschritt = !mEinzelschritt;
    print_to_stderr(_sprintf("Debug: Single step mode = %d\n",
			     mEinzelschritt));
    if (!mEinzelschritt) {
    ...
    if (mGModus==gmodus_spiel) {
      ...
      print_to_stderr("Debug: Replace fall\n");
    }

  } else if (buch == 'L') {
    if (mGModus == gmodus_kein_spiel) {
      print_to_stderr("Debug: Load log file\n");
      Aufnahme::laden(getLogName());
      setSpielerModus(Aufnahme::getSpielerModus());
      mIntLevelName = Aufnahme::getLevelName();
      mLevelNr = ld->getLevelNr(mIntLevelName);
      if (mLevelNr==0) mIntLevelName="";
      PrefsDaten::setLastLevel(mIntLevelName);

      // TRANSLATORS: The %d is the level number
      print_to_stderr(_sprintf("Level %s (%d)\nFor replay start level.\n",
			       mIntLevelName.data(), mLevelNr));
      mAbspielen = true;
    } else
      print_to_stderr("Debug: Do *not* load log file during the game.\n");
    ...
    if (mGModus == gmodus_spiel) {
      mGModus = gmodus_warte_gewonnen;
      print_to_stderr("Debug: Win instantly\n");
    }
    
  } else if (buch == 'h') {
    print_to_stderr("Debug: Help (all keys to be combined with Alt)\n"
		    "  b: Receive greys\n"
    ...
    if (mGModus == gmodus_kein_spiel) {
      print_to_stderr("Debug: Reload levelconf\n");
      try {
   	ld->ladLevelSummary(true,berechneVersion());
      } catch (Fehler fe) {
	printFehler(2, "Could not reload the level description file because of the following error:\n",
	            fe);
      }
    } else
      print_to_stderr("Debug: Do *not* load log file during the game.\n");
    ...
    print_to_stderr(_sprintf("Debug: Test of row-exchange = %d\n",
			     mRueberReihenTest));
    
  } else if (buch == 'v') {
    mSchnellvorlauf = !mSchnellvorlauf;
    print_to_stderr(_sprintf("Debug: Fast forward = %d\n", mSchnellvorlauf));
    
  } else if (buch == 'z') {
    mZeitlupe = !mZeitlupe;
    print_to_stderr(_sprintf("Debug: Slow motion = %d\n", mZeitlupe));
...
void malNaechstesFall(int sp) {
  mSpielfeld[sp]->malUpdateNaechstesFall();
}
...
int getLetzterLevel() {
  return ld->getLevelNr(mIntLevelName);
}
...
void setzeVersion(const Version & version) {
  ...
  /* Achtung! Jetzt enth�lt mZusatzVersionen erstmal zu viel.
     Aber der �berschu� wird gleich rausgel�scht. */
  ...
  catch(Str konflikt) {throw Fehler("Conflicting versions: %s",
					 konflikt.data());}
}
