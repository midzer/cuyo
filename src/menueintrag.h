...
/* Malt in der Zukunft einen Teil eines Men�eintrags.
   Das Bezugssystem f�r Koordinaten ist AlignTop im Men�eintrag. */
...
public:
  int mBinSubBereich;
  int mX0,mX1; // mX1 ist wie immer um 1 zu gro�
  int mY0,mY1;
  int mXPos;
  bool mAbschneiden;
...
class MenuEintrag {
 ...
 public:
  ...
  const int mHoehe;  /* Ganz viel wird einfacher, wenn das hier const ist,
			also verl�sst sich auch ganz viel drauf.
			Im Moment scheinen wir auch nicht mehr zu brauchen.
			Wenn sich das mal �ndert, kann man sich immer noch
 ...
 protected:
  ...
  int mAccIndex;
  void (*mDoReturn)();
  bool (*mGetStrom)(); /* Ein Men�punkt, der grad keinen Strom kriegt,
                          kann auch nicht ausgew�hlt werden */
  Art mArt;
  int mX0, mX1;
 ...
 public:
  ...
  /* y muss nicht �berpr�ft werden; liegt auf jeden Fall zwischen 0 und hoehe */
 ...
 protected:
  void doPapiEscape();  /* Damit auch die Erben das d�rfen. */
  void doPapiNavigiere(int);     /* ebenso */

  /* Das f�llt DrawDinge auf, malt aber nicht selbst.
     Wird auch aufgerufen, um die Breite des Men�punkts zu bestimmen */
...
/* K�nnte man Sch�nfinkeln, w�re diese Klasse fast unn�tig... */
...
class MenuEintragAuswahlmenu: public MenuEintragSubmenu {
  ...
  int mVorlauf;  // Wie viele Eintr�ge hat das Men� vor dem "ersten"?
