...
DrawDing::DrawDing(const Str & text, int hotkey,
   	           int binSubBereich, /* Subbereich, zu dem dieses Drawding geh�rt */
                   int x, int y,
		   int align /*= AlignHCenter*/,   /* Akzeptiert nur waagerechtes Zeug,
				       senkrecht ist immer zentriert. */
                   Font * font, /*= NULL*/  /* Default h�ngt von aktSubBereich ab */
  ...
  switch (align) {
    ...
    case AlignHCenter:
      ...
      break;
    default: throw iFehler("%s","Invalid alignment");
  }
  #if HOT_ALIGN
      if (hotkey >= 0) {
...
/* Wenn man nicht m�chte, dass das Bild zur Gr��e des Hintergrundrahmens
   beitr�gt, w�re es vielleicht inzwischen sauberer, binsubbereich auf
...
/* Aufrufen, wenn sich m�glicherweise der Stromstatus ge�ndert hat */
...
void MenuEintrag::anzeigen(int x, int y, bool graue) {
  ...
  /* Unsch�n, k�nnte irgendwann mal ein Bug werden, tut aber im Moment:
     Eigentlich m�sste ein Men�Eintrag informiert werden, wenn es sich
     �ndert, ob die Grauen neben ihm sitzen oder nicht, damit er ein
     ...
     Eintrag ver�ndert wird, so dass die erforderlichen Graphik-Updates
     sowieso geschehen */
...
void MenuEintragEscape::doReturn(bool) {
  /* Sound wird gespielt, wenn das Men� sich zumacht. */
  doPapiEscape();
}
...
void MenuEintragAuswahl::doReturn(bool) {
  ...
  /* Vorsicht: Im Moment *muss* das folgende in dieser Reihenfolge
     ...
     die Graphik des Levelpack-Men�eintrags geupdatet. Der neue
     Levelpack muss davor schon ausgew�hlt worden sein. */
  mDoReturnInt(mArg);
  doPapiEscape();
}
...
int MenuEintragAuswahlmenu::getMausPos(int x, int y) {
  ...
  /* Manuel zusammengeflicktes befindet-sich-die-Maus-�ber-einem-Pfeil.
...
//   if (mSubBereich != subbereich_keiner) {
//     /* Erstmal deaktiviert, weil's mir nicht so gef�llt
...
void MenuEintragAuswahlmenu::doPfeil(int d) {
  ...
  /* Abstand zwischen altem und neuem Text bestimmen. Soll eigentlich
     ...
     schon vorher reingeguckt h�tte oder umgekehrt */
...
/* Wo ist diese Variable besser gekapselt? Wenn sie ein private Member von
   ...
   (und ein "static" hat, damit der Linker auch nicht verr�t, dass es diese
   Variable gibt)? */
...
void MenuEintragSpielerModus::doReturn(bool) {
  ...
  /* Da steht im Moment die Funktion drin, die Restart-last-level
     neu malt (wegen Strom�nderung).
     Eigentlich k�nnte man hieraus auch gleich einen generischen
     Ankreuzlisten-Men�EintragTyp machen, im Stil der anderen Auswahlmen�s. */
  (*mDoWechsel)();
...
void MenuEintragSound::doReturn(bool) {
  /* Da es auf manchen Rechnern ziemlich lange dauert, bis SDL merkt, dass
     es den Sound doch nicht anschalten kann, f�r diese Wartezeit eine optische
     R�ckkopplung */
  mBitteWarten = true;
  updateDrawDinge();
